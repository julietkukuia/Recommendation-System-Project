    ...
    if model_path and meta_path:
        try:
            ...
            with open(meta_path) as f:
                meta = json.load(f)
            return model, meta, "hybrid" if meta.get("feature_set") == "hybrid_svd" else "baseline_model"
...
def load_embeddings():
    ...
    if not (u_path and v_path):
        # Optional: look for shards (not required for baseline)
...
def try_load_events(uploaded, default_path: str) -> pd.DataFrame:
    ...
    try:
        if default_path.lower().endswith(".parquet"):
            return pd.read_parquet(os.path.join(APP_DIR, default_path))
        return pd.read_csv(os.path.join(APP_DIR, default_path))
    except Exception:
        pass

    # fallback: shards named events_enriched_part*.parquet (optional)
...
# default to sample_events.parquet in the repo
